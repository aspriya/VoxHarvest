...
const api = {
  ...
  exportDataset: (projectPath: string, items: any[], format: 'f5' | 'piper' | 'xtts' | 'fish', speakerName?: string) => ipcRenderer.invoke('export-dataset', projectPath, items, format, speakerName),
  getSettings: () => ipcRenderer.invoke('get-settings'),
  saveSettings: (settings: Settings) => ipcRenderer.invoke('save-settings', settings),
  selectDirectory: () => ipcRenderer.invoke('select-directory'),
  generateText: (prompt: string, count: number, systemPromptOverride?: string, modelId?: string) => ipcRenderer.invoke('generate-text', prompt, count, systemPromptOverride, modelId),
  getModels: (provider: 'openai' | 'gemini') => ipcRenderer.invoke('get-models', provider),
